...
RealSquareMatrix::RealSquareMatrix(const RealSquareMatrix &m):
	Matrix<GLdouble>(m), _lu_decomposition_is_done(m._lu_decomposition_is_done),
...
GLboolean RealSquareMatrix::ResizeRows(GLuint row_count) {
	return Matrix::ResizeRows(row_count)&&
			Matrix::ResizeColumns(row_count);
}
GLboolean RealSquareMatrix::ResizeColumns(GLuint column_count){
	return Matrix::ResizeRows(column_count)&&
			Matrix::ResizeColumns(column_count);
}
